...
#-----------------copied from base.py at : /Users/asr2031/programs/anaconda/lib/python2.7/site-packages/msmbuilder/ ---
...
class tICA(BaseEstimator, TransformerMixin):
    ...
    def _solve(self):
        ...
        print("\n\t**********TICA: used rhs = self.covariance_ + (self.gamma / self.n_features) * np.trace(self.covariance_) * np.eye(self.n_features)   with gamma 0.05*******\n")
        self.gamma = 0.05                                                                                                                 #ASGHAR Apr10,2018
        rhs = self.covariance_ + (self.gamma / self.n_features) * np.trace(self.covariance_) * np.eye(self.n_features)                    #ASGHAR Apr10,2018
    ...
    def solve2(self, pca_cutoff=0):                                                          #ASGHAR Apr 9, 2018
        self.cov_mat = self.covariance_
        self.timelag_corr_mat = self.offset_correlation_
        print("using PCA based tICA")
        """
...
#        self._sort()                                      #ASGHAR Apr 9 , 2018
  	       
	# sort in order of decreasing value                #ASGHAR Apr 9 , 2018
        ind = np.argsort(vals)[::-1]
        vals = vals[ind]
        vecs = vecs[:, ind]
    ...
    def _sort(self):                                        #ASGHAR Arp 9, 2018
        """
    ...
    def fit(self, sequences, y=None):
        ...
        """
        self._initialized = False
#        check_iter_of_sequences(sequences, max_iter=3)  # we might be lazy-loading		## Asghar, June 4, 2019
        for X in sequences:
            self._fit(X)
