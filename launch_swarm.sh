...
swarmNumber=0
numberOfTrajsPerSwarm=40
firstSubjob=1  # should be number of next subjob to be run. For example, if you haven't 
...
lastSubjob=100 # obviously this MUST be larger that $first_subjob
...
do
  ...
  then
     job_scheduler_output="$(sbatch -J $jobName -N1 -n1 -p $partitionName --cpus-per-task=1 --mem=10G --gres=gpu:1 -t 0-02:00:00 -o ./raw_swarms/submission_logs/${fullJobName}_slurm-%A_%a.out --array=0-${indexed_num_of_trajs} ./submit_swarm_subjobs.sh $swarmNumber $numberOfTrajsPerSwarm $subjob)"       
  else
     job_scheduler_output="$(sbatch --depend=afterok:${job_scheduler_number} -J $jobName -N1 -n1 -p $partitionName --cpus-per-task=1 --mem=10G --gres=gpu:1 -t 0-02:00:00 -o ./raw_swarms/submission_logs/${fullJobName}_slurm-%A_%a.out --array=0-${indexed_num_of_trajs} ./submit_swarm_subjobs.sh $swarmNumber $numberOfTrajsPerSwarm $subjob)" 
  fi
