...
do
  ...
  then
     job_scheduler_output="$(sbatch -J $jobName -N1 -n1 -p $partitionName --cpus-per-task=1 --mem=20G --gres=gpu:1 -t 0-00:02:00 -o ./submission_logs/slurm-%A_%a.out --array=0-${indexed_num_of_trajs} ./submit_swarm_subjobs.sh $swarmNumber $numberOfTrajsPerSwarm $subjob)"       
  else
     job_scheduler_output="$(sbatch --depend=afterok:${job_scheduler_number} -J $jobName -N1 -n1 -p $partition_name --cpus-per-task=1 --mem=20G --gres=gpu:1 -t 0-00:02:00 -o ./submission_logs/${fullJobName}_slurm-%A_%a.out --array=0-${indexed_num_of_trajs} ./submit_swarm_subjobs.sh $swarmNumber $numberOfTrajsPerSwarm $subjob)" 
  fi
